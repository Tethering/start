...
function modifier_earth_arrow_thinker_lua:OnCreated( kv )	
	...
	if IsServer() then
		...
		self:GetParent():AddAbility("earth_arrow_lua")
		local abil = self:GetParent():FindAbilityByName("earth_arrow_lua")
		abil:SetLevel(1)
		self:GetParent():CastAbilityOnPosition(self:GetCaster():GetOrigin(), abil, self:GetParent():GetPlayerOwnerID())
	end
end
